...
/* -----------------------------
  ✅ Bloque 5.1 — Semántica determinista (sin IA)
  - Servicios inteligentes (si faltan)
  - Categorías/cards inteligentes (especialmente ecommerce)
----------------------------- */
...
function smartEcommerceCategories({ sector, servicesList, target, businessName }) {
  const s = norm(sector);
  const t = norm(target);
  ...
  const base = [
    ...
    { title: "Accesorios", description: "Calcetines, cuidados y extras." },
  ];

  // heurísticas por keywords
  if (s.includes("zapat") || s.includes("footwear") || s.includes("sneaker")) {
    return base;
  }
...
function smartCardsForDomain({ domain, businessType, servicesList, sector, target, primaryGoal }) {
  // Ecommerce: categorías premium
  if (businessType === "ecommerce" || domain === "ecommerce") {
    return smartEcommerceCategories({ sector, servicesList, target });
  }

  // Hospitality: “especialidades”
  if (domain === "hospitality") {
    return [
  ...
  // Health: “tratamientos”
  if (domain === "health") {
    // si vienen servicios del usuario, convertimos 3–5 en cards “tratamiento”
    const fromServices = uniqueLimit(
      servicesList.map((x) => titleize(x)),
      5
    ).map((t) => ({ title: t, description: "Valoración y plan claro desde la primera visita." }));
  ...
  // Generic fallback: 3 cards basadas en servicios si hay
...
function buildCopyV1({ sector, location, target, goal, businessType, personality, servicesList }) {
  ...
  // Si hay servicios, mete una frase corta “de catálogo” determinista (sin IA)
  const servicesHint =
...
export async function generateSiteV2(clientBrief) {
  ...
  // ✅ Servicios semánticos: si el usuario no mete, generamos defaults por dominio
  ...
  const faq = buildFAQ(domain, primaryGoal);
  ...
  const spec = {
    ...
    modules: {
      ...
      // ✅ Bloque 5.1: cards semánticas (sin “Opción 1/2/3”)
      cards_auto: {
