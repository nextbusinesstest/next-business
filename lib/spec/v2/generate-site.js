...
function pickPersonality(businessType, goal, sector) {
  // fallback (la personalidad real la aplica /api/generate-site con auto_personality)
  const g = goal?.primary_goal || "";
...
// ✅ Bloque 4.2: arquetipo por goal
function pickArchetype(primaryGoal) {
...
/**
 * ✅ Bloque 4.2: layouts goal-driven + hero split para SaaS landing
 */
function buildHomeSectionsForGoal(goal, pack, archetype) {
  const g = goal?.primary_goal || "capture_leads";

  // Ecommerce (pack especial) — lo dejamos como estaba
  ...
  // ✅ Landing / acción única (SaaS): hero split
  if (g === "single_action") {
    const heroVariant = archetype === "saas_landing_v1" ? "hero_saas_split_v1" : "hero_product_minimal_v1";
  ...
  // ✅ Reservas / citas: hero → services → testimonials → faq → steps → contact
...
export async function generateSiteV2(clientBrief) {
  ...
  const archetype = pickArchetype(primaryGoal);
