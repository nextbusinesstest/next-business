...
function formatSingleActionNote(detail) {
  ...
  // Si el usuario ya mete un verbo ("Solicitar demo", "Pedir presupuesto", etc.)
  const startsWithVerb =
    /^(solicita(r)?|pide(r)?|reserva(r)?|contacta(r)?|obt(e|é)n(er)?|descarga(r)?|agenda(r)?)\b/i.test(d);

  if (startsWithVerb) {
    // Caso típico: "Solicitar demo" -> mejor gramática:
    // "Solicita una demo..." (no "Solicitar demo y ...")
  ...
  // Si no empieza con verbo: "Demo", "Presupuesto", etc.
...
function buildCopyV1({
  ...
  // Contact note por objetivo (FIX: single_action usa goal_detail correctamente sin duplicar)
  const contactNote =
    g === "book_appointments"
...
export async function generateSiteV2(clientBrief) {
  ...
  // ✅ Steps por objetivo / dominio (premium ligero)
  const g = goal?.primary_goal || "capture_leads";
  const stepsItems =
    g === "book_appointments"
      ? [
