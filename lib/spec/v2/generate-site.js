...
function buildCopyV1({ businessName, sector, location, target, goal, businessType, personality }) {
  ...
  const contactNote =
    g === "book_appointments"
      ...
      : g === "single_action"
        ? (safeTrim(goal?.goal_detail) ? `Solicita ${goal.goal_detail} y te respondemos en breve.` : "Solicita una demo y te respondemos en breve.")
        : g === "sell_online"
  ...
  const spec = {
    ...
    modules: {
      ...
      bullets_auto: {
        ...
        items: (copy.bullets || ["Profesionalidad", "Respuesta rápida", "Calidad y confianza"]).slice(0, 4).map((t) => ({
          title: t,
        })),
      },
