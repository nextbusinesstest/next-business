...
function buildHomeSectionsForGoal(goal, pack, archetype) {
  ...
  if (g === "single_action") {
    ...
    // IMPORTANTE: PacksRouter no soporta bullets_saas_checks_v1 ni services_grid_saas_v1.
    // Para estabilidad, SOLO usamos variantes soportadas.
    return [
  ...
  // ✅ Reservas / citas (solo variantes soportadas)
  if (g === "book_appointments") {
    // IMPORTANTE: PacksRouter no soporta bullets_trust_inline_v1 ni services_grid_booking_v1.
    return [
      { module: "hero", variant: "hero_product_minimal_v1", props_ref: "modules.hero_auto" },
      { module: "bullets", variant: "bullets_auto_v1", props_ref: "modules.bullets_auto" },
      { module: "services", variant: "services_grid_auto_v1", props_ref: "modules.services_auto" },
...
function buildStepsForSaasIntent(intent) {
  ...
  if (intent === "quote") {
    return [
      { title: "Define alcance", description: "Equipo, volumen e integraciones." },
  ...
  // demo/default
  return [
    { title: "Solicita demo", description: "Vemos tu caso y objetivos." },
    { title: "Setup rápido", description: "Ajustes mínimos para tu equipo." },
    { title: "Primer impacto", description: "Automatiza y mide resultados." },
  ];
}
...
export async function generateSiteV2(clientBrief) {
  ...
  // ✅ Bloque 6: override desde Lab (si viene), si no, automático
  const requestedPersonality = safeTrim(clientBrief?.brand_personality);
  const personality = requestedPersonality || pickPersonality(businessType, goal, sector);
  ...
  const spec = {
    ...
    layout: {
      pack,
      archetype,
      // ✅ Bloque 6.2: header/footer por personalidad/arquetipo
      header_variant:
    ...
    modules: {
      ...
      contact_auto: {
        ...
        cta_label: primaryGoal === "book_appointments" ? "Reservar cita" :
                   primaryGoal === "sell_online" ? "Contactar" : "Enviar mensaje",
      },
    },

    contact: {
      phone: safeTrim(clientBrief?.contact_phone),
      email: safeTrim(clientBrief?.contact_email),
      whatsapp: safeTrim(clientBrief?.contact_whatsapp),
      address: location || "",
    },
