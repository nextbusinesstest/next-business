...
function applyColorOverrides(vars, colors) {
  ...
  // IMPORTANT:
  // Por defecto NO queremos que spec.brand.design_tokens.colors machaque el theme.
  // Esto estaba causando que personalidades "dark" quedaran en blanco/negro.
  // Solo aplicamos overrides si el spec lo marca explícitamente.
  const wantsOverride =
    colors._override === true ||
    colors.__override === true ||
    colors.override_theme === true ||
    String(colors.mode || "").toLowerCase() === "override";

  if (!wantsOverride) return vars;
  const v = { ...vars };
